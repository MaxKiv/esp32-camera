  ...
  do {                                                                         \
    if (++(counter) == 1) {                                                    \
      ...
      ESP_CAMERA_ETS_PRINTF(                                                   \
          DRAM_STR("cam_hal: %s - 100 additional misses\r\n"), first);         \
    }                                                                          \
    if ((counter) == 10000)                                                    \
      (counter) = 1;                                                           \
  } while (0)
...
static int cam_verify_jpeg_soi(const uint8_t *inbuf, uint32_t length) {
  ...
  CAM_WARN_THROTTLE(warn_soi_miss_cnt, "NO-SOI - JPEG start marker missing");
  return -1;
}
...
static int cam_verify_jpeg_eoi(const uint8_t *inbuf, uint32_t length,
  ...
  if (search_forward) {
    ...
    while (i + 4 <= length) {
      ...
      uint32_t m = (~x & (x - ONE)) &
                   HIGH; /* mask has high bit set for candidate bytes */
...
void IRAM_ATTR ll_cam_send_event(cam_obj_t *cam, cam_event_t cam_event,
                                 BaseType_t *HPTaskAwoken) {
  if (xQueueSendFromISR(cam->event_queue, (void *)&cam_event, HPTaskAwoken) !=
      pdTRUE) {
    ll_cam_stop(cam);
    cam->state = CAM_STATE_IDLE;
#if CAM_LOG_SPAM_EVERY_FRAME
    ESP_DRAM_LOGD(TAG, "EV-%s-OVF",
                  cam_event == CAM_IN_SUC_EOF_EVENT ? "EOF" : "VSYNC");
#else
    static uint16_t ovf_cnt = 0;
    CAM_WARN_THROTTLE(ovf_cnt, cam_event == CAM_IN_SUC_EOF_EVENT
                                   ? "EV-EOF-OVF"
                                   : "EV-VSYNC-OVF");
#endif
  }
...
static void cam_task(void *arg) {
  ...
  while (1) {
    ...
    case CAM_STATE_READ_BUF: {
      ...
      size_t pixels_per_dma =
          (cam_obj->dma_half_buffer_size * cam_obj->fb_bytes_per_pixel) /
      ...
      if (cam_event == CAM_IN_SUC_EOF_EVENT) {
        ...
        if (!cam_obj->psram_mode) {
          ...
          frame_buffer_event->len += ll_cam_memcpy(
              cam_obj, &frame_buffer_event->buf[frame_buffer_event->len],
        ...
        if (cam_obj->jpeg_mode && cnt == 0) {
          if (cam_obj->psram_mode) {
            ...
            int soi_off = cam_verify_jpeg_soi(frame_buffer_event->buf,
        ...
        if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
          if (cam_obj->jpeg_mode) {
            if (!cam_obj->psram_mode) {
              if (cam_obj->fb_size <
                  (frame_buffer_event->len + pixels_per_dma)) {
                ESP_CAMERA_ETS_PRINTF(DRAM_STR("cam_hal: FB-OVF\r\n"));
                cnt--;
              } else {
                frame_buffer_event->len += ll_cam_memcpy(
...
static lldesc_t *allocate_dma_descriptors(uint32_t count, uint16_t size,
                                          uint8_t *buffer) {
  lldesc_t *dma =
      (lldesc_t *)heap_caps_malloc(count * sizeof(lldesc_t), MALLOC_CAP_DMA);
  if (dma == NULL) {
    return dma;
  }
...
static esp_err_t cam_dma_config(const camera_config_t *config) {
  ...
  cam_obj->dma_node_cnt = (cam_obj->dma_buffer_size) /
                          cam_obj->dma_node_buffer_size; // Number of DMA nodes
  cam_obj->frame_copy_cnt =
      cam_obj->recv_size /
      cam_obj->dma_half_buffer_size; // Number of interrupted copies, ping-pong
                                     // copy
  if (cam_obj->psram_mode) {
    cam_obj->frame_copy_cnt++;
  }
  ...
  ESP_LOGI(TAG,
           "buffer_size: %d, half_buffer_size: %d, node_buffer_size: %d, "
           "node_cnt: %d, total_cnt: %d",
           (int)cam_obj->dma_buffer_size, (int)cam_obj->dma_half_buffer_size,
           (int)cam_obj->dma_node_buffer_size, (int)cam_obj->dma_node_cnt,
           (int)cam_obj->frame_copy_cnt);

  cam_obj->dma_buffer = NULL;
  cam_obj->dma = NULL;

  cam_obj->frames = (cam_frame_t *)heap_caps_aligned_calloc(
      alignof(cam_frame_t), 1, cam_obj->frame_cnt * sizeof(cam_frame_t),
      MALLOC_CAP_DEFAULT);
  ...
  for (int x = 0; x < cam_obj->frame_cnt; x++) {
    ...
    ESP_LOGI(TAG, "Allocating %d Byte frame buffer in %s", alloc_size,
             _caps & MALLOC_CAP_SPIRAM ? "PSRAM" : "OnBoard RAM");
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(4, 3, 0)
    // In IDF v4.2 and earlier, memory returned by heap_caps_aligned_alloc must
    // be freed using heap_caps_aligned_free. And heap_caps_aligned_free is
    // deprecated on v4.3.
    cam_obj->frames[x].fb.buf =
        (uint8_t *)heap_caps_aligned_alloc(16, alloc_size, _caps);
#else
    cam_obj->frames[x].fb.buf = (uint8_t *)heap_caps_malloc(alloc_size, _caps);
#endif
    CAM_CHECK(cam_obj->frames[x].fb.buf != NULL, "frame buffer malloc failed",
              ESP_FAIL);
    if (cam_obj->psram_mode) {
      // align PSRAM buffer. TODO: save the offset so proper address can be
      // freed later
      cam_obj->frames[x].fb_offset =
          dma_align - ((uint32_t)cam_obj->frames[x].fb.buf & (dma_align - 1));
      cam_obj->frames[x].fb.buf += cam_obj->frames[x].fb_offset;
      ESP_LOGI(TAG, "Frame[%d]: Offset: %u, Addr: 0x%08X", x,
               cam_obj->frames[x].fb_offset,
               (unsigned)cam_obj->frames[x].fb.buf);
      cam_obj->frames[x].dma = allocate_dma_descriptors(
          cam_obj->dma_node_cnt, cam_obj->dma_node_buffer_size,
          cam_obj->frames[x].fb.buf);
      CAM_CHECK(cam_obj->frames[x].dma != NULL, "frame dma malloc failed",
                ESP_FAIL);
    }
  ...
  if (!cam_obj->psram_mode) {
    ...
    if (NULL == cam_obj->dma_buffer) {
      ESP_LOGE(TAG,
               "%s(%d): DMA buffer %d Byte malloc failed, the current largest "
               "free block:%d Byte",
               __FUNCTION__, __LINE__, (int)cam_obj->dma_buffer_size,
               (int)heap_caps_get_largest_free_block(MALLOC_CAP_DMA));
      return ESP_FAIL;
    }

    cam_obj->dma = allocate_dma_descriptors(cam_obj->dma_node_cnt,
...
esp_err_t cam_config(const camera_config_t *config, framesize_t frame_size,
  ...
  ret = ll_cam_set_sample_mode(cam_obj, (pixformat_t)config->pixel_format,
                               config->xclk_freq_hz, sensor_pid);
...
#endif
  ESP_LOGI(TAG, "PSRAM DMA mode %s",
           cam_obj->psram_mode ? "enabled" : "disabled");
  ...
  if (cam_obj->jpeg_mode) {
    ...
    cam_obj->recv_size =
        cam_obj->width * cam_obj->height * cam_obj->in_bytes_per_pixel;
    cam_obj->fb_size =
  ...
  CAM_CHECK_GOTO(cam_obj->event_queue != NULL, "event_queue create failed",
                 err);
  ...
  cam_obj->frame_buffer_queue =
      xQueueCreate(frame_buffer_queue_len, sizeof(camera_fb_t *));
  CAM_CHECK_GOTO(cam_obj->frame_buffer_queue != NULL,
                 "frame_buffer_queue create failed", err);

  ret = ll_cam_init_isr(cam_obj);
  CAM_CHECK_GOTO(ret == ESP_OK, "cam intr alloc failed", err);

#if CONFIG_CAMERA_CORE0
  xTaskCreatePinnedToCore(cam_task, "cam_task", CAM_TASK_STACK, NULL,
                          configMAX_PRIORITIES - 2, &cam_obj->task_handle, 0);
#elif CONFIG_CAMERA_CORE1
  xTaskCreatePinnedToCore(cam_task, "cam_task", CAM_TASK_STACK, NULL,
                          configMAX_PRIORITIES - 2, &cam_obj->task_handle, 1);
#else
  xTaskCreate(cam_task, "cam_task", CAM_TASK_STACK, NULL,
              configMAX_PRIORITIES - 2, &cam_obj->task_handle);
#endif
...
void cam_start(void) { ll_cam_vsync_intr_enable(cam_obj, true); }
...
camera_fb_t *cam_take(TickType_t timeout) {
  ...
  for (;;) {
    TickType_t elapsed = xTaskGetTickCount() -
                         start; /* TickType_t is unsigned so rollover is safe */
    if (elapsed >= timeout) {
    ...
    if (cam_obj->jpeg_mode) {
      ...
      if (cam_obj->psram_mode) {
        ...
        size_t probe_len =
            eoi_probe_window(cam_obj->dma_node_buffer_size, dma_buffer->len);
    ...
    skip_eoi_check:
      ...
      CAM_WARN_THROTTLE(warn_eoi_miss_cnt, "NO-EOI - JPEG end marker missing");
      cam_give(dma_buffer);
      ...
      dma_buffer->len = ll_cam_memcpy(cam_obj, dma_buffer->buf, dma_buffer->buf,
                                      dma_buffer->len);
    }
...
bool cam_get_psram_mode(void) { return g_psram_dma_mode; }
